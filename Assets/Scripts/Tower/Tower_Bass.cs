...
public class Tower_Bass : Tower {
    ...
    void Attack()
    {
        ...
        {
            if (_NowChanging)
                _MyAnimation.SetAnimation("changeattack", 6);
            else
                _MyAnimation.SetAnimation("attack", 6);
            _NowTime -= getNowAttackDelayTime();
    ...
    IEnumerator AttackDelay()
    {
        ...
        if (_TargetMonster != null)
        {
            ...
            obj.GetComponent<AttackProjectile>().Init(getNowDamage(), _TargetMonster, 800,3);
            //Debug.Log("BassShoot");
        }
        else
            _NowTime = getNowAttackDelayTime();
